...
public class Autonomous {
    ...
    public StateMachine theWalkOfShame = getStateMachine(autoStage)//auto for an "in case all else fails"
            .addState(() -> (robot.crane.setElbowTargetPos(3300,1.0)))
            .addState(() -> (robot.driveForward(true, .6, .60)))//moves up
            .build();
    public StateMachine primaryBlueMec = getStateMachine(autoStage)
            ...
            .addState(() -> (robot.rotateIMU(270, 9))) //rotate back toward building zone - stay in 2nd column
            ...
            .addState(() -> (robot.driveForward(true, .5, .60))) //todo: should continue until proximity from back wall detected
            .addState(() -> (robot.rotateIMU(0, 9))) //turn to foundation
            .addState(() -> (robot.driveForward(true, .1, .60)))
            .addTimedState(3,//starts the gripper and pciks up the block
                    () -> robot.crane.ejectStone(),
                    () -> robot.crane.stopGripper())
            .addState(() -> (robot.crane.hookOn()))
            .addState(() -> (robot.driveForward(false, .5, .30)))
            //.addState(() -> (robot.driveStrafe(.5,.6)))
